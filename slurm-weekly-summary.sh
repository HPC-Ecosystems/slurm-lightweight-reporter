...
while IFS= read -r line; do
    ...
    printf "  Job %-6s User: %-10s State: %-10s Symbol: %s\n" "$jobid" "$user" "$state" "$symbol"
done <<< "$JOBS" >> "$REPORT"

echo "" >> "$REPORT"
echo "Symbol Summary:" >> "$REPORT"
for sym in "✅" "❌" "🚫" "⚠️"; do
    count=${symbol_count[$sym]:-0}
    printf "  %s : %s jobs\n" "$sym" "$count"
done >> "$REPORT"
echo "" >> "$REPORT"
...
echo "Symbolic Job Status Mapping:" >> "$REPORT"
...
declare -A cpu_sec_alt mem_sec_alt
...
while IFS= read -r line; do
    ...
    start=$(echo "$line" | grep -oP 'StartTime=\K[0-9\-T:]+')
    end=$(echo "$line" | grep -oP 'EndTime=\K[0-9\-T:]+')

    start_sec=$(date -d "$start" +%s)
    end_sec=$(date -d "$end" +%s)
    runtime_sec=$(( end_sec - start_sec ))

    (( runtime_sec <= 0 )) && continue

    ((cpu_sec_alt["$user"] += cpu * runtime_sec))
    ((mem_sec_alt["$user"] += mem * runtime_sec))
done <<< "$JOBS"

echo "Alternate Resource Summary (StartTime–EndTime based):" >> "$REPORT"
for user in "${!cpu_sec_alt[@]}"; do
    ch=$(awk "BEGIN { printf \"%.2f\", ${cpu_sec_alt[$user]}/3600 }")
    gbh=$(awk "BEGIN { printf \"%.2f\", ${mem_sec_alt[$user]}/3600/1024 }")
...
for user in "${!cpu_sec[@]}"; do
    cpu_hr=$(awk "BEGIN { printf \"%.6f\", ${cpu_sec[$user]}/3600 }")
    mem_hr=$(awk "BEGIN { printf \"%.6f\", ${mem_sec[$user]}/3600/1024 }")
...
# Top Users by CPU-Seconds
echo "Top 10 Users by CPU-Seconds:" >> "$REPORT"

for user in "${!cpu_sec[@]}"; do
    echo -e "${cpu_sec[$user]}\t$user"
done | sort -rn | head -n 10 | awk '{printf "  %-10s %s CPU-seconds\n", $2, $1}' >> "$REPORT"
